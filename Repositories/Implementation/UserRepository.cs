...
(@c_userName,@c_email,@c_password,@c_address,@c_gender,@c_mobile,@c_image)", _conn);
                        com.Parameters.AddWithValue("@c_userName", data.c_UserName); com.Parameters.AddWithValue("@c_email", data.c_Email);
            ...
            {
                await _conn.CloseAsync();
                Console.WriteLine("Register Failed , Error :- " + e.Message); return -1; //-1 if There is error during registration 
            }
        }
